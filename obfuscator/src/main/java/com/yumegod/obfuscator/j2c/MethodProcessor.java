...
import com.yumegod.obfuscation.VMProtectMutation;
import com.yumegod.obfuscation.VMProtectUltra;
import com.yumegod.obfuscation.VMProtectVirtualization;
import com.yumegod.obfuscator.YumeCloudProtection;
import com.yumegod.obfuscator.j2c.specials.ClInitSpecialMethodProcessor;
import com.yumegod.obfuscator.j2c.specials.DefaultSpecialMethodProcessor;
import com.yumegod.obfuscator.j2c.specials.SpecialMethodProcessor;
import com.yumegod.obfuscator.enums.VMProtectOption;
import com.yumegod.obfuscator.j2c.handlers.*;
import com.yumegod.obfuscator.utils.Util;
import com.yumegod.obfuscator.j2c.caches.CatchesBlock;
...
public class MethodProcessor {
    ...
    public static final ArrayList<String>[] methodsToCheck = new ArrayList[]{new ArrayList<String>(), new ArrayList<String>(), new ArrayList<String>(), new ArrayList<String>()};
    ...
    private final YumeCloudProtection obfuscator;
    private final InstructionHandlerContainer<?>[] handlers;

    public MethodProcessor(YumeCloudProtection obfuscator) {
    ...
    public static String getClassGetter(MethodContext context, String desc) {
        if (desc.startsWith("[")) {
            return "env->FindClass(VMProtectDecryptStringA(\"" + desc + "\"))";
        }
        if (desc.endsWith(";")) {
    ...
    public void processMethod(MethodContext context) {
        if (!context.method.name.equals("<clinit>") && !context.method.name.equals("<init>")) {
            methodsToCheck[0].add(context.clazz.name);
            methodsToCheck[1].add(context.method.name);
            methodsToCheck[2].add(context.method.desc);
            methodsToCheck[3].add((context.method.access & Opcodes.ACC_STATIC) != 0 ? "true" : "false");
        }
        ...
        methodName = "ProtectedByYumeCloud_" + methodName.replace('/', '_');
        ...
        if (context.proxyMethod != null) {
            ...
            context.nativeMethods.append(String.format("                    { %s, %s, (void *)&%s },\n",
                    "const_cast<char*>(VMProtectDecryptStringA(\"" + context.method.name + "\"))",
                    "const_cast<char*>(VMProtectDecryptStringA(\"" + context.method.desc + "\"))", methodName));
        }
        ...
        VMProtectOption option = VMProtectOption.None;

        int weight = 0;
        if (context.clazz.invisibleAnnotations != null) {
            for (AnnotationNode invisibleAnnotation : context.clazz.invisibleAnnotations) {
                if (invisibleAnnotation.desc.equals(Type.getDescriptor(VMProtectUltra.class))) {
                    weight += 3;
                } else if (invisibleAnnotation.desc.equals(Type.getDescriptor(VMProtectVirtualization.class))) {
                    weight += 2;
                } else if (invisibleAnnotation.desc.equals(Type.getDescriptor(VMProtectMutation.class))) {
                    weight += 1;
                }
            }
        }
        ...
        if (methodWeight != 0 && weight != methodWeight) {
            weight = methodWeight;
        }

        if (weight >= 3) {
            option = VMProtectOption.Ultra;
        } else if (weight == 2) {
            option = VMProtectOption.Virtualization;
        } else if (weight == 1) {
            option = VMProtectOption.Mutation;
        }

        switch (option) {
            case Ultra:
                output.append("    VMProtectBeginUltra(\"" + context.clazz.name + "_" + method.name + "_YumeCloudProtection_" + index + "\");").append("\n");
                break;
            case Virtualization:
                output.append("    VMProtectBeginVirtualization(\"" + context.clazz.name + "_" + method.name + "_YumeCloudProtection_" + index + "\");").append("\n");
                break;
            case Mutation:
                output.append("    VMProtectBeginMutation(\"" + context.clazz.name + "_" + method.name + "_YumeCloudProtection_" + index + "\");").append("\n");
                break;
        }
        ...
        output.append("    if (env->ExceptionCheck()) { ").append(String.format("return (%s) 0;",
                CPP_TYPES[context.ret.getSort()])).append(" }\n");
        output.append("    if (classloader == nullptr) { env->FatalError(").append(
                "VMProtectDecryptStringA(\"" + "classloader == null" + "\")").append(String.format("); return (%s) 0; }\n", CPP_TYPES[context.ret.getSort()]));
        output.append("\n");
        if (!isStatic) {
        ...
        if (weight > 0) {
            index++;
            output.append("    VMProtectEnd();").append("\n");
        }
        output.append("}\n\n");
